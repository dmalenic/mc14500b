...
export function init4BitComparator(programSelector) {
    pgmInitCurrentProgram(programSelector, `4-bit Comparator`,
        ...
        Set IN5 to ON and IN6 to OFF to load IN1-IN4 into RAM0-RAM3 <small>(the 2nd op. is B1-B3).</small><br/>
        Set IN5 to OFF and IN6 to OFF to load IN1-IN4 <small>(the 1st operand is A1-A4).</small><br/>
        Set IN6 to ON to perform the comparison. If A=B, then the output OUT0 is set.<br/>
        If A&gt;B, then the output OUT1 is set. If A&lt;B, then the output OUT2 is set.`,
        rom, 0);
}
